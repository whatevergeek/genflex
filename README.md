...
GenFlex checks for an existing `genflex-docs/genflex-state.md`, reports in-progress work, and dispatches only the requested feature. State and audit work do not implicitly start party mode.

## Layout and customization

```text
.genflex/
├── agents/       default party roster; project `genflex-docs/agents/` overrides it
...
Use `--docs-path <path>` to change the generated-documents directory. Templates in `<docs-path>/templates/` take precedence over `.genflex/templates/`. Add custom party agents under `genflex-docs/agents/` without changing GenFlex files.
