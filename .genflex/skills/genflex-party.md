...
1. Discover Markdown agents in this order: `<project-root>/sdd-docs/agents/*.md`, then `<project-root>/.genflex/agents/*.md`. Project files with the same name override defaults. If neither location contains agents, use the built-in seven-agent roster: Winston, Mary, John, Amelia, Sally, Paige, and Murat.
...
Users can add Markdown agents under `sdd-docs/agents/`. Each should identify name, title, icon, role, and behavior. No registry edit is required.
