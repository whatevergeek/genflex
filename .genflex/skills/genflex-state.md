# genflex-state — ad hoc phase tracking

Use for ad hoc tracking, phase tracking, resume, or any user-defined progress list. GenFlex does not prescribe phases or a development workflow. Default docs path is `<project-root>/genflex-docs/`. In the same chat message as the request, use `--docs-path <path>` to choose another directory.
